...
async def get_audio(db: Session, audio_id: int):
    return db.query(models.Audio).filter(models.Audio.id == audio_id).first()

async def get_audios(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.Audio).offset(skip).limit(limit).all()

async def create_audio(db: Session, audio: schemas.AudioCreate, user_id: int):
...
async def get_jobs(db: Session, skip: int = 0, limit: int = 100):
    return db.query(models.Job).offset(skip).limit(limit).all()

async def create_job(db: Session, job: schemas.JobCreate, audio_id: int):
...
async def create_transcript(db: Session, transcript: schemas.TranscriptCreate, job_id: int):
